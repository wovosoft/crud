...
use Wovosoft\Crud\Exceptions\RuntimeException;
...
//        this copies dirs which has files. We need to copy empty dirs too
    ...
    protected function copyStubs($packagePath, $package, $packageFolderName)
    {
        ...
        foreach (config('wovosoft-crud.directories_to_create') as $directory_to_create) {
            if (!File::exists("$packagePath/$directory_to_create")) {
                if (!File::makeDirectory("$packagePath/$directory_to_create", 0755, true)) {
                    throw new RuntimeException("Cannot create $directory_to_create folder");
                }
                $this->info("$directory_to_create Folder Created");
            }
        }
    ...
    {
        $path = config("wovosoft-crud.$configName");
